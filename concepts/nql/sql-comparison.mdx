...
EXPLAIN SELECT user_id, email FROM users WHERE age > 30
```

```sql NQL EXPLAIN
-- Shows: estimated row count, estimated cost, data freshness
EXPLAIN SELECT user_id, email FROM company_data."123" WHERE age > 30
```
...
|---------|--------|-------------|
| `SELECT *`, `COUNT(*)` | Not supported | [List columns explicitly](/reference/nql/explicit-columns) |
